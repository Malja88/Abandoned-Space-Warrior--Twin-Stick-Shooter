...
public partial class HealingSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        Entities
            .WithoutBurst()
            .ForEach((Entity entity, HealTag tag, ref DynamicBuffer <StatefulTriggerEvent> triggerEventBuffer) =>
            {
                for (int i = 0; i < triggerEventBuffer.Length; i++)
                {
                    ...
                    if (triggerEvent.State == StatefulEventState.Enter && tag.HealAction is IAbility ability)
                    {
                        ability.Execute();
